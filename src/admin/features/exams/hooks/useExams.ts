...
export const useExams = () => {
  ...
  const openEdit = (exam: IAdminExam) => {
    ...
    setEditing(exam); setForm({ name: exam.name, slug: exam.slug, categoryId: catId, description: exam.description, icon: exam.icon, color: exam.color, bannerUrl: exam.bannerUrl || '', isActive: exam.isActive, order: exam.order, group: exam.group || '', sectionId: exam.sectionId || '', subCategories: exam.subCategories || [] }); setModalOpen(true);
  };
